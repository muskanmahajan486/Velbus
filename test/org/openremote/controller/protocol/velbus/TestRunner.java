...
class TestRunner {
  ...
  void Run() {
    ...
    log.debug("Connecting to the Velbus network...");
  
    // Instantiate the builder - add to applicationContext and let Spring do this
    VelbusCommandBuilder cmdBuilder = new VelbusCommandBuilder("mdar-model.no-ip.biz;mdar-model.no-ip.biz", "8446;8446", "org.openremote.controller.protocol.velbus.VelbusIpConnection"); 
    
//    // Build a command to initialise the connection manager
//    cmdBuilder.build(createCommandElement(10, Action.STATUS, ""));
//    VelbusConnectionManager connectionManager = cmdBuilder.getConnectionManager();            
//       
//    // Wait for connection
...
//    int passed = 0;
//    
//    while (passed < timeout && connectionManager.getConnectionStatus() != ConnectionStatus.CONNECTED)
//    {      
//      passed += 1000;
//      try {
//        Thread.sleep(1000);
...
//    System.out.println("Connected");
    ...
    // Build commands and sensors
      ...
      for (int addr : dimmerAddresses) {
        ...
        TestSensor sensor = new TestSensor("TEST", 0, null, command, null, EnumSensorType.CUSTOM);
        ...
        TestSensor sensor2 = new TestSensor("TEST", 0, null, command, null, EnumSensorType.CUSTOM);
...
//    loopRead(sensors, 1, 1000);
  ...
  private static void loopRead(TestSensor[] sensors, int loops, int pause) {
    int count = 0;
  ...
  private static void singleRead(TestSensor[] sensors) {
    String output = "";
    String header = "";
  ...
  private static void sendCommand(VelbusWriteCommand command) {
    command.send();
    try {
      Thread.sleep(1000);
    } catch (InterruptedException e) {
  ...
  private static Element createCommandElement(int networkId, int address, VelbusCommand.Action action, String value, Integer dynamicValue) {
