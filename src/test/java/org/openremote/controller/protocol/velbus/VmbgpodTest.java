...
public class VmbgpodTest extends BaseTest {
    VelbusCommandBuilder builder;

    @Override
    VelbusCommandBuilder getCommandBuilder() {
        if (builder == null) {
            builder = new VelbusCommandBuilder(new String[] {""}, new String[] {"6060"});
        }
        return builder;
    }

    @Test
    public void initialiseGpodDevice() throws InterruptedException {
        VelbusReadCommand tempCommand = (VelbusReadCommand)getCommandBuilder().build(createCommandElement(1, 224, VelbusCommand.Action.TEMP_STATUS));
